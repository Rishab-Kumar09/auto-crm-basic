  ...
  const { data: ratingsData } = useQuery({
    ...
    queryFn: async () => {
      ...
      const averageRating = totalRatings > 0
        ? validFeedback.reduce((sum, f) => sum + f.rating, 0) / totalRatings
        : 0;

      // Calculate rating distribution
      const distribution = validFeedback.reduce((acc, f) => {
      ...
      <div className="space-y-3">
        {[5, 4, 3, 2, 1].map((rating) => (
          <div key={rating} className="flex items-center gap-2">
            ...
            <Progress 
              value={getPercentage(ratingsData?.distribution?.[rating as keyof RatingDistribution] || 0)} 
              className="h-2"
            />
